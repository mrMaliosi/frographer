...
class Summarizer:
    ...
    async def _generate_response(self, prompt: str, options: dict = None) -> str:
        """
    ...
    async def summarize_messages(self, messages):
        ...
        prompt = (
            ...
            "ВАЖНО: Не используй Markdown-разметку (никаких заголовков с #, жирного текста с **, курсива или списков с *). "
            "Используй только обычный текст, заглавные буквы для выделения разделов и переносы строк для структурирования.\n\n"
    ...
    async def characterize_user(self, username, messages, activity_stats=None):
        """
        Анализирует сообщения пользователя для создания характеристики через Ollama.
        messages: Список строк (тексты сообщений)
        activity_stats: Строка с информацией об активности (например, количество сообщений)
        """
        if not messages:
        ...
        prompt = (
            ...
            f"{stats_context}"
            "\n\nВАЖНО: Не используй Markdown-разметку (никаких заголовков с #, жирного текста с **, курсива). "
            "Ответ должен быть представлен в виде чистого текста с использованием переносов строк.\n\n"
    ...
    async def generate_yandex_praise(self) -> str:
        ...
        prompt = (
            ...
            "Постарайся придумать что-то оригинальное и необычное."
        )
        return await self._generate_response(prompt, options={"temperature": 0.9})
    ...
    async def generate_yandex_criticism(self) -> str:
        ...
        prompt = (
            ...
            "Постарайся быть изобретательным в своей иронии."
        )
        return await self._generate_response(prompt, options={"temperature": 0.9})
